...
<script lang="ts">
  ...
  onMount(() => {
    internalTree.forEach((_, gi) => {
      internalTree[gi].btCollapse = new Collapse(
        internalTree[gi].el.querySelector(".filter-group-items-container"),
        { toggle: false }
      );
    });
  });
    ...
    {#each internalTree as group}
      <li bind:this={group.el} class="filter-group">
        <header on:click={toggle.bind(null, group.id)}>
          <span
            class:bi-dash-square={!group.collapsed}
            class:bi-plus-square={group.collapsed}
          />
        ...
        <div class="filter-group-items-container collapse show">
          <ul>
            {#each group.filters as filter}
              <li
                class="filter-item"
                class:chosen={filter.chosen}
                on:click={choose.bind(null, group.id, filter.id)}
              >
                <span class:bi-arrow-right-short={filter.chosen} />
                {#if filter.name}
